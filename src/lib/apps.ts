...
function appDetailBySelector(selector: Record<string, unknown>) {
  if (typeof selector.client_id !== "undefined") {
    return ApplicationsService.getAppByClientIdAppsByClientIdClientIdGet((selector as ClientIdObject).client_id)
  } else if (typeof selector.appid !== "undefined") {
    return ApplicationsService.getAppByAppidAppsByIdAppidGet((selector as AppIdObject).appid)
  } else {
    throw new TypeError(`expect an object with field "client_id" or "appid", got ${selector}`)
  }
}

export function getAppDetail(client: ClientConfig, selector: ClientIdObject): Fork<NotFoundError, App>
export function getAppDetail(client: ClientConfig, selector: AppIdObject): Fork<NotFoundError, App>
