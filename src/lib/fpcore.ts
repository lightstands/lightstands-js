...
export function lmap<L, R, LN>(fn: ((l: L) => LN), e: Either<L, R>): Either<LN, R> {
  return either({
    left: (v) => fn(v),
    right: (r) => r,
  }, e)
...
export async function aeither<L, R, LN, RN>(matcher: EitherMatcher<L, R, LN, RN>, fork: Fork<L, R>): Fork<LN, RN> {
  const v = await fork
  return either(matcher, v)
}

export async function armap<L, R, RN>(fn: (val: R) => RN, fork: Fork<L, R>): Fork<L, RN> {
  const v = await fork
  return rmap(fn, v)
}
...
export async function almap<L, R, LN>(fn: (val: L) => LN, fork: Fork<L, R>): Fork<LN, R> {
  const v = await fork
  return lmap(fn, v)
}

export function fold<E, C>(folder: (prev: C, next: E) => C, initial: C, values: Iterable<E>): C {
  let prev = initial
...
export function* map<E, N>(fn: (val: E) => N, values: Iterable<E>): Iterable<N> {
  for (const el of values) {
    yield fn(el)
  }
}

export function* filter<E>(fn: (val: E) => boolean, values: Iterable<E>): Iterable<E> {
  for (const el of values) {
    if (fn(el)) {
      yield el
    }
...
export function* typeFilter<E, N extends E>(fn: (val: E) => val is N, values: Iterable<E>): Iterable<N> {
  for (const el of values) {
    if (fn(el)) {
      yield el
    }
...
export function filterMap<T, N>(fn: (val: T) => Maybe<N>, values: Iterable<T>): Iterable<N> {
  return map(unboxRight, typeFilter(something, map(fn, values)))
}

export function flatten<L, R>(e: Either<Either<L, R>, Either<L, R>>): Either<L, R> {
  return e.value
}
